...
TODO:
* For New Shephard, show sub-orbital flights as hashed background:
    background: repeating-linear-gradient(45deg, lightgreen, lightgreen 10px, #01a201 10px, #01a201 20px);

 */

const
    url = new URL(document.location.href),
    SHOW_LEGEND = true && url.searchParams.get('SHOW_LEGEND') !== 'false',
    PAGE_LOAD_DATETIME = new Date(),
    // Period of the beat in milliseconds for "Dream Escape."
    AUDIO_DREAM_ESCAPE_PERIOD_MS = 1096,
    TIMELINE_HEIGHT = 100,
    FAST_MODE = false || url.searchParams.get('FAST_MODE') === 'true',
    // TIME_MULTIPLE = FAST_MODE ? 10 : 100,
    // TIME_MULTIPLE = FAST_MODE ? 10 : 75,
    // TIME_MULTIPLE = FAST_MODE ? 10 : 60,
    // TIME_MULTIPLE = FAST_MODE ? 10 : 40,
    // TIME_MULTIPLE = FAST_MODE ? 15 : 60,
    TIME_MULTIPLE = FAST_MODE ? 15 : 40,
    ORBIT_CLASSES = ['leo', 'sso', 'heo', 'geo', 'lunar'],
    ORBIT_CLASSES_STRUCTS = {
        'leo': {
            label: 'LEO',
            rgb: '128, 255, 128',
            lineRgb: '0, 128, 0',
    ...
    ASTRONAUT_EMOJI = '\u{1F468}\u{200d}\u{1f680}',  // '\u{1F9D1}',
    DROPLETS_EMOJI = '\u{1F4A6}',
    EXPLOSION_EMOJI = '\u{1F4A5}',
    FLAME_EMOJI = '\u{1F525}',
    ROCKET_EMOJI = '\u{1F680}',
    SNOWFLAKE_EMOJI = '\u2744',
    TROPHY_EMOJI = '\u{1F3C6}',
    FREQUENTIST_PAYLOADS = {
  "SSO": 1898.875,
  "GEO": 6963.451612903225,
  "L1": 1906.0,
  "Lunar": 307.0,
  "Heliocentric": 2137.25,
  "HEO": 1310.6666666666667,
  "MEO": 3124.242424242424,
  "GTO": 6963.451612903225,
  "HTO": 1360.0,
  "ISS": 4328.793103448276,
  "LEO": 7398.1578947368425,
  "L2": 2105.0,
  "Sub-orbital": 6947.5,
},
    USD_INFLATION = {
...
// Returns a closure that will execute only once when an animation is over.
function getAnimOnceClosure(closure) {
    var finished = false;
    return (anim) => {
        if (!finished && anim.progress == 100) {
            finished = true;
            closure();
        }
    };
...
function getLaunchId(rocket, opt_prefix) {
    return (opt_prefix ? opt_prefix : '') + 'launch-' + rocket.header.replace(/[\W\.]/g, '-');
}


function getPrunedLaunchArray(launches) {
...
function didRocketLaunch(rocket) {
    return rocket.launch_outcome_class != 'precluded';
}
...
# SOURCE: https://gist.githubusercontent.com/mikhailov-work/ee72ba4191942acecc03fe6da94fc73f/raw/dccf2856de98e3c60c7f0ac8acf3c33d9726dfb8/turbo_colormap.py
...
function interpolate(colormap, x) {
    const xx = Math.max(0.0, Math.min(1.0, x)),
        a = Math.floor(xx * 255.0),
        b = Math.min(255, a + 1),
        f = xx * 255.0 - a;
...
function turbo_interpolate(x) {
    const color01 = interpolate_or_clip(turbo_colormap_data, x);
    return [Math.round(255 * color01[0]), Math.round(255 * color01[1]), Math.round(255 * color01[2])];
}

function interpolate_or_clip(colormap, x) {
    if (x < 0.0) {
        return [0.0, 0.0, 0.0];
    } else if (x > 1.0) {
        return [1.0, 1.0, 1.0];
...
// Fills in the div with legend colors and labels.
function populateLegend(divId, labels) {
    $(`#${divId}`).html(
        `<center>` +
        _.map(labels, (l) => {
            return `<span class="legend-wrapper">` +
                    `<span class="legend-swatch" style="background-color: ${l.color}"></span>` +
...
function showLegendStartAnimation(closure) {
    if (SHOW_LEGEND) {
        $('#legend').show();
        setTimeout(() => {
            anime({
